...
def _random_data(max_cluster):
    sr = 20000
    nspikes = 10000
    spike_samples = np.cumsum(np.random.exponential(scale=0.025, size=nspikes))
...
def test_ccg_symmetry_time():
    ...
    spike_samples_1 = np.cumsum(np.r_[np.arange(1), np.diff(spike_samples)[::-1]])
