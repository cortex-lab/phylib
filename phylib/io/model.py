...
class TemplateModel(object):
    ...
    def _find_file_format(self):
        if self._find_path('templates.waveforms.*npy', mandatory=False):
            self.file_format = 'alf'
        elif self._find_path('templates.npy', mandatory=False):
            self.file_format = 'ks2'
        else:
            raise IOError("Missing files or unknown file format.")
        logger.info("File format is %s.", self.file_format)
    ...
    def _load_data(self):
        ...
        if self.template_amplitudes is not None:
            assert self.template_amplitudes.shape == (self.n_templates,)
    ...
    def _load_templates(self):
        ...
        if self.file_format == 'alf':
            ...
            path = self._find_path('templates.waveforms.*npy')
            ...
            path = self._find_path('templates.waveformsChannels*.npy')
            ...
            path = self._find_path('templates.npy', mandatory=False)
            if path is None:
                raise IOError("Template waveforms are mandatory.")
            assert path
            data = self._read_array(path)
            logger.debug("Open KS2 template waveforms.")
            data = np.atleast_3d(data)
            assert data.ndim == 3
            assert data.dtype in (np.float32, np.float64)
            ...
            if path is None:
                ...
                cols = self._read_array(path).astype(np.int32)
                if cols.ndim != 2:  # pragma: no cover
                    cols = np.atleast_2d(cols).T
                assert cols.ndim == 2
            ...
            for n in np.arange(n_templates):
                ...
                # Extract the sub whitening matrix.
                mat = self.wmi[np.ix_(channel_ids, channel_ids)]
                assert mat.shape == (n_channels_tmp,) * 2
                assert np.sum(np.isnan(mat.ravel())) == 0
                assert template_w.shape == (n_samples, n_channels_tmp,)

                # Unwhiten the template waveform.
                channel_ids0 = channel_ids.copy()
                template_u = np.matmul(template_w, mat)
                assert template_u.shape == (n_samples, n_channels_tmp)
                ...
                amplitude = template_u.max(axis=0) - template_u.min(axis=0)
                assert amplitude.shape == (n_channels_tmp,)
                assert np.all(amplitude >= 0)
                has_signal = amplitude > amplitude.max() * self.amplitude_threshold
                ...
                # Reorder template channels.
                channel_ids = channel_ids[reorder]
                assert channel_ids.shape == (n_channels_tmp,)

                # Reorder template data.
                template_u = template_u[:, _index_of(channel_ids, channel_ids0)]
                assert template_u.shape == (n_samples, n_channels_tmp)
                assert np.all(np.diff(template_u.max(axis=0) - template_u.min(axis=0)) <= 0)

                # Set the template indices.
                cols[n, :n_channels_tmp] = channel_ids
                cols[n, n_channels_tmp:] = -1

                # Set the templates.
                templates_wfs[n, :, :n_channels_tmp] = template_u
                templates_wfs[n, :, n_channels_tmp:] = 0

                # Check the decreasing amplitude.
                assert not np.all(template_u == 0)
    ...
    def get_depths(self):
        ...
        while True:
            ...
            if self.sparse_features.cols is not None:
                ichannels = self.sparse_features.cols[self.spike_templates[ispi]].astype(np.int32)
            else:
